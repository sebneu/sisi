package interpreter.tree;

import interpreter.tree.types.EnumEntryType;
import interpreter.tree.types.MType;
import interpreter.tree.types.Type;
import interpreter.tree.values.Tuple;
import interpreter.tree.values.Value;

import java.util.HashMap;
import java.util.Map;

public class Scope {

    private Scope parent;
    private Map<String, Value> variables;
    private Map<String, Type> types;
    private Map<Tuple, Integer> sentToAll;

    public Scope() {
	// only for the global scope, the parent is null
	this(null);
    }

    public Scope(Scope p) {
	parent = p;
	variables = new HashMap<String, Value>();
	types = new HashMap<String, Type>();
	sentToAll = new HashMap<Tuple, Integer>();
    }

    public void declare(String var, Type type) {
	if (resolve(var) != null) {
	    // There is already such a variable, throw exception
	    throw new RuntimeException("Identifier name '" + var
		    + "' is already declared.");
	} else {
	    if (type.isSet()) {
		// Sets have no initialization
		variables.put(var, new Value(new TupleSet()));
	    } else {
		// A newly declared variable
		variables.put(var, Value.NULL);
	    }
	    types.put(var, type);
	}
    }
    
    public void changeType(String var, Type type) {
	if (resolve(var) == null) {
	    // There is already such a variable, throw exception
	    throw new RuntimeException("Identifier name '" + var
		    + "' is not declared.");
	} else {
	    types.put(var, type);
	}
}

    public Scope copy() {
	// Create a shallow copy of this scope. Used in case functions are
	// are recursively called. If we wouldn't create a copy in such cases,
	// changing the variables would result in changes to the Maps from
	// other "recursive scopes".
	Scope s = new Scope();
	s.variables = new HashMap<String, Value>(this.variables);
	s.types = new HashMap<String, Type>(this.types);
	return s;
    }

    public boolean isGlobalScope() {
	return parent == null;
    }

    public Scope parent() {
	return parent;
    }

    public void assign(String var, Value value) {
	if (resolve(var) != null) {
	    // There is already such a variable, re-assign it
	    this.reAssign(var, value);
	} else {
	    // no such variable
	    throw new RuntimeException("No variable '" + var
		    + "' has been declared.");

	}
    }

    private void reAssign(String identifier, Value value) {
	if (variables.containsKey(identifier)) {
	    if (value.hasType(types.get(identifier))) {
		// The variable is declared in this scope
		variables.put(identifier, value);
	    } else {
		throw new RuntimeException("the variable " + identifier
			+ " is of type " + types.get(identifier)
			+ " and cannot be assigned value " + value);
	    }
	} else if (parent != null) {
	    // The variable was not declared in this scope, so let
	    // the parent scope re-assign it
	    parent.reAssign(identifier, value);
	}
    }

    public Value resolve(String var) {
	Value value = variables.get(var);
	if (value != null) {
	    // The variable resides in this scope
	    return value;
	} else if (!isGlobalScope()) {
	    // Let the parent scope look for the variable
	    return parent.resolve(var);
	} else {
	    // Unknown variable
	    return null;
	}
    }

    public boolean isInitialized(String var) {
	Value value = variables.get(var);
	if (value != null) {
	    // The variable resides in this scope
	    return !value.isNull();
	} else if (!isGlobalScope()) {
	    // Let the parent scope look for the variable
	    return parent.isInitialized(var);
	} else {
	    // Unknown variable
	    return false;
	}
    }

    /*
    public void sendTo(Value receiver, TupleNode tn) {
	Tuple tuple = tn.eval();
	if (receiver == Value.ALL) {
	    boolean exists = false;
	    for (Tuple t : sentToAll.keySet()) {
		if (t.equals(tuple)) {
		    int oldValue = sentToAll.get(t);
		    oldValue++;
		    sentToAll.put(t, oldValue);
		    exists = true;
		    break;
		}
	    }
	    if (!exists) {
		sentToAll.put(tuple, 1);
	    }
	} else {
	    throw new RuntimeException("send to " + receiver
		    + " is not implemented");
	}
    }
    */

    @Override
    public String toString() {
	StringBuilder b = new StringBuilder();
	for (Map.Entry<String, Value> entry : variables.entrySet()) {
	    if (!(types.get(entry.getKey()) instanceof EnumEntryType
		    || types.get(entry.getKey()) instanceof MType)) {
		b.append(entry.getKey() + ": " + entry.getValue().toString()
			+ "\n");
	    }
	}
	if (!sentToAll.isEmpty()) {
	    b.append("sent to all: " + sentToAll + "\n");
	}
	return b.toString();
    }

    public void remove(String id, Type type) {
	if (types.get(id).equals(type)) {
	    variables.remove(id);
	    types.remove(id);
	} else {
	    throw new RuntimeException("wrong type of " + id
		    + " for removing of declaration");
	}
    }
}
